...
### menu choice o: open the csv file of movies in emacs for editing, & load your custom lisp functions for data entry
	...
	o)
	    ...
	    echo "  C-c g  -> Move cursor to a field in the present record by choosing field name in minibuffer (my/book-goto-field)"
	    echo "  C-c u  -> Stamp today's date in lastUpdated field of record at point (my/book-stamp-updated-today)"
	    echo "  C-c d  -> Interactively prompt for lastRead date.  (my/book-lastRead-date-entry)"
	    echo "  C-c n  -> Jump to bottom and insert new record (my/book-insert-new-record)"
	    echo "  C-c c  -> edit comment (Capture Mode) - (my/book-edit-comment-indirect)"
	    echo "            * Use C-c C-c to SAVE and EXIT capture"
	    echo "            * Use C-c C-k to DISCARD changes"
	    echo "  C-c q  -> add to, or remove from, (toggle) queue (0/1) - (my/book-toggle-queue)"
	    echo "  C-c t  -> add a tag - (my/book-tag-add)"
	    ...
	    echo "  C-c h  -> print out a cheatsheet for these lisp functions in a new emacs buffer (my/book-help-workbench)"
	    echo "=================================================="
